...
public class TypeIterator extends AbstractResourceVisitor implements Iterator<Resource> {

    Function<Resource, Boolean> rFilter;
    Collection<Resource> internalList = new ArrayList<>();
    Iterator<Resource> internalIT;

    public TypeIterator(Function<Resource, Boolean> filter, Resource root) {
        rFilter = filter;
        accept(root);
    }
